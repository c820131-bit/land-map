...
# 기존 867e827 커밋의 index.html에서 데이터 추출
import subprocess
...
.legend {{
  ...
  border: 1px solid var(--border); border-radius: 10px; padding: 10px 14px; z-index: 1000;
}}
...
<div class="loading" id="loading">
  ...
  <div class="loading-txt">지도 로딩 중...</div>
</div>
...
<div class="panel">
  ...
  <div class="panel-body">
    ...
    <div class="toggle on" data-layer="inside" onclick="tog('inside')">
      <div class="toggle-icon inside">📍</div>
      <div class="toggle-info"><div class="toggle-name">지구내</div><div class="toggle-cnt">{cnt_inside:,}건</div></div>
      <div class="toggle-sw"></div>
    </div>
    <div class="toggle on" data-layer="nearby" onclick="tog('nearby')">
      <div class="toggle-icon nearby">📌</div>
      <div class="toggle-info"><div class="toggle-name">지구 인근</div><div class="toggle-cnt">{cnt_nearby:,}건</div></div>
      <div class="toggle-sw"></div>
    </div>
    <div class="section">일반 경공매</div>
    <div class="toggle" data-layer="auction" onclick="tog('auction')">
      <div class="toggle-icon auction">🔵</div>
      <div class="toggle-info"><div class="toggle-name">경매</div><div class="toggle-cnt">{cnt_auction:,}건</div></div>
      <div class="toggle-sw"></div>
    </div>
    <div class="toggle" data-layer="public" onclick="tog('public')">
      <div class="toggle-icon public">🟣</div>
      <div class="toggle-info"><div class="toggle-name">공매</div><div class="toggle-cnt">{cnt_public:,}건</div></div>
      <div class="toggle-sw"></div>
    </div>
    <div class="section">인프라</div>
    <div class="toggle" data-layer="proj" onclick="tog('proj')">
      <div class="toggle-icon infra">🏗️</div>
      <div class="toggle-info"><div class="toggle-name">사업지구 경계</div><div class="toggle-cnt">172개</div></div>
      <div class="toggle-sw"></div>
    </div>
    <div class="toggle" data-layer="rail" onclick="tog('rail')">
      <div class="toggle-icon infra">🚇</div>
      <div class="toggle-info"><div class="toggle-name">철도역</div><div class="toggle-cnt">280개</div></div>
      <div class="toggle-sw"></div>
    </div>
    <div class="toggle" data-layer="highway" onclick="tog('highway')">
      <div class="toggle-icon infra">🛣️</div>
      <div class="toggle-info"><div class="toggle-name">고속도로 IC</div><div class="toggle-cnt">159개</div></div>
      <div class="toggle-sw"></div>
    </div>
    <div class="map-btns">
    ...
    ['inside', 'nearby', 'auction', 'public'].forEach(function(type) {{
      DATA[type].forEach(function(d) {{
        ...
        kakao.maps.event.addListener(m, 'click', function() {{
          var info = '<div style="padding:12px;max-width:280px;font-size:12px;line-height:1.5">' +
            '<b style="font-size:14px">' + d.case_no + '</b><br>' +
            '<span style="color:#666">' + d.address + '</span><br><br>' +
            '<b>용도:</b> ' + d.usage + '<br>' +
            '<b>감정가:</b> <span style="color:#e11d48">' + fmtPrice(d.appraisal) + '</span><br>' +
            '<b>최저가:</b> <span style="color:#e11d48">' + fmtPrice(d.min_price) + '</span> (' + d.ratio + ')<br>' +
            '<b>상태:</b> ' + d.status + ' | ' + d.date + '</div>';
...
function tog(layer) {{
  ...
  var el = document.querySelector('[data-layer="' + layer + '"]');
  el.classList.toggle('on', visible[layer]);
...
function createInfra() {{
  ...
  projData.forEach(function(p) {{
    ...
    var poly = new kakao.maps.Polygon({{
      path: path, strokeWeight: 2, strokeColor: '#06b6d4', strokeOpacity: 0.8,
  ...
  railData.forEach(function(r) {{
    ...
    var m = new kakao.maps.Marker({{
      ...
      image: new kakao.maps.MarkerImage(svg, new kakao.maps.Size(20, 20))
    }});
    layers.rail.push(m);
  }});
  ...
  highwayData.forEach(function(h) {{
    ...
    var m = new kakao.maps.Marker({{
      ...
      image: new kakao.maps.MarkerImage(svg, new kakao.maps.Size(18, 12))
    }});
  ...
  fetch('/.netlify/functions/vworld?bbox=' + bbox)
    ...
    .then(function(data) {{
      ...
      curPoly = new kakao.maps.Polygon({{
        path: path, strokeWeight: 3, strokeColor: COLORS[type], strokeOpacity: 1,
        fillColor: COLORS[type], fillOpacity: 0.4, map: map
      }});
    }})
...
function doSearch() {{
  ...
  if (found) {{
    if (!visible[found.t]) tog(found.t);
...
print(f"새 index.html 생성 완료! ({cnt_total:,}건)")
